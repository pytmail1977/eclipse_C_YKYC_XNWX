...
drop table if exists GJFW_LRXJ_SLT;

drop table if exists GJFW_LRXJ_XJZT;

drop table if exists GJFW_LRXJ_YST;
...
create table GJFW_LRXJ_SLT
(
   SLTWJ_ID             int not null auto_increment,
   ZPWJ_ID              int not null,
   SLTWJ_MC             varchar(50),
   SLTWJ_LJ             varchar(4096),
   primary key (SLTWJ_ID)
);
...
create table GJFW_LRXJ_YST
(
   YSTWJ_ID             int not null auto_increment,
   ZPWJ_ID              int not null,
   YSTWJ_MC             varchar(50),
   YSTWJ_LJ             varchar(4096),
   YSTWJ_LX             tinyint comment '0:jpeg,1:png',
   primary key (YSTWJ_ID)
);
...
(
   ...
   ZPWJ_SLT             longblob comment '备用（缩略图存成字段还是作为文件单独存放待定）',
   primary key (ZPWJ_ID)
);
...
alter table GJFW_LRXJ_SLT add constraint FK_Relationship_6 foreign key (ZPWJ_ID)
      references GJFW_LRXJ_ZP (ZPWJ_ID) on delete restrict on update restrict;

alter table GJFW_LRXJ_YST add constraint FK_Relationship_7 foreign key (ZPWJ_ID)
      references GJFW_LRXJ_ZP (ZPWJ_ID) on delete restrict on update restrict;
